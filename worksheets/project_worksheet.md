...
`(This is made available through the use of the frontend web application and the backend server. Jibaboom can access the Compute basic/advance result by directly calling the backend API endpoints. The administrator can also access this functionality through the web application.)`
-   Q2. Consider the use case of `Insert basic/advance data`. **HOW** is the data stored? 
`(It is stored in the appropriate database.)`
...
```
how can frontend get data from backend?
```

Think about how you can do it and type your answer below:

```
[The frontend can send a HTTP request with an Ajax call to the appropriate API endpoints in the backend server. The backend server then processes the request, retrieving the appropriate data from the database and sending it back to the frontend in the HTTP response.]
```
